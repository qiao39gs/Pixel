...
export default function ZoomControls() {
  ...
  return (
    <div className="absolute bottom-3 right-3 z-30 flex items-center gap-0.5 glass-toolbar rounded-xl px-1 py-1 animate-toolbar-pop">
      ...
      <span className="px-2 text-xs font-mono font-bold text-slate-200 select-none w-14 text-center">{zoomPct}%</span>
      ...
      <button onClick={() => window.dispatchEvent(new CustomEvent('pixel:reset-view'))} title="还原视图" className="p-2 rounded-lg text-slate-200 hover:bg-white/15 transition-colors cursor-pointer"><Focus className="w-4 h-4" /></button>
    </div>
  );
}