...
interface Props {
  ...
  editMode: boolean;
  setBrushBead: (v: BeadPaletteItem) => void;
  selectedBeadHighlight: string | null;
  setSelectedBeadHighlight: (v: string | null) => void;
  transformedPixels: TransformedPixel[];
}

export default function StatsPanel({ stats, editMode, setBrushBead, selectedBeadHighlight, setSelectedBeadHighlight, transformedPixels }: Props) {
  return (
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center pb-3.5 border-b border-slate-100 mb-5 gap-3">
        <div className="flex flex-col">
          <h3 className="font-display font-bold text-[#1D1D1F] text-sm flex items-center gap-2 leading-none">
            <Grid3X3 className="w-4 h-4 text-indigo-500" />MARD 标准色卡
          </h3>
        ...
        return (
          <div key={group.series} className="mb-5">
            <div className="flex items-center justify-between mb-2.5 px-1">
              <span className="text-[11px] font-bold text-slate-500 uppercase tracking-wider">{group.name}</span>
              <span className="text-[10px] text-slate-400 font-mono">{seriesStats.reduce((sum, s) => sum + s.count, 0)} 颗 · {seriesStats.length} 色</span>
            </div>
              ...
              {seriesStats.map(statItem => {
                ...
                return (
                  <div key={statItem.bead.code}
                    onClick={() => editMode ? setBrushBead(statItem.bead) : setSelectedBeadHighlight(isSelected ? null : statItem.bead.code)}
                    ...
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-full shadow-inner border border-black/[0.04] flex items-center justify-center font-mono font-bold text-[10px]"
                      ...
                      <div className="flex flex-col">
                        ...
                        <span className="text-[10px] text-slate-400 mt-0.5 font-mono">#{statItem.bead.code}</span>
                      </div>
                    </div>
