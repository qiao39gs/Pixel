      ...
      {COLOR_GROUPS.map(group => {
        ...
        return (
          <div key={group.series} className="mb-5">
            <div className="flex items-center justify-between mb-2.5 px-1"><span className="text-[11px] font-bold text-slate-500 uppercase tracking-wider">{group.name}</span><span className="text-[10px] text-slate-400 font-mono">{seriesStats.reduce((sum, s) => sum + s.count, 0)} 颗 · {seriesStats.length} 色</span></div>
                ...
                return (
                  <div key={statItem.bead.code} onClick={() => editMode ? setBrushBead(statItem.bead) : setSelectedBeadHighlight(isSelected ? null : statItem.bead.code)}
                    ...
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-full shadow-inner border border-black/[0.04] flex items-center justify-center font-mono font-bold text-[10px]" style={{ backgroundColor: statItem.bead.hex, color: ['#FFFFFF','#F5F5F5','#FFE0B2'].includes(statItem.bead.hex) ? '#334155' : '#FFFFFF' }}>{statItem.bead.code}</div>
                      <div className="flex flex-col"><span className="font-bold text-slate-900 text-xs leading-none">{statItem.bead.name}</span><span className="text-[10px] text-slate-400 mt-0.5 font-mono">#{statItem.bead.code}</span></div>
                    </div>
