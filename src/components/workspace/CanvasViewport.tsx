  ...
  return (
    <div className={`bg-slate-950 border border-[#1D1D21] rounded-3xl p-6 shadow-2xl flex flex-col relative overflow-hidden min-h-[calc(100dvh-130px)] md:min-h-[500px] max-h-[calc(100dvh-80px)] transition-all ${mobileTab !== 'canvas' ? 'hidden lg:block' : ''}`}>
      <div className="flex justify-between items-center gap-2 mb-3 z-10">
        <h1 className="text-sm font-bold text-white tracking-wide">编辑器</h1>
        <div className="flex items-center gap-2 flex-wrap justify-end">
          <span className="text-xs font-bold px-2.5 py-1 bg-white/[0.06] border border-white/[0.04] rounded-lg text-slate-300 font-mono">{gridWidth} × {gridHeight}</span>
          <span className="text-xs text-slate-400 font-semibold">· <strong className="text-indigo-400">{stats.length} 色</strong></span>
      ...
      <div className="flex flex-wrap justify-center items-center gap-1.5 mb-4 pb-3 border-b border-white/[0.04] z-10">
        <button onClick={() => { setEditMode(!editMode); setBrushBead(null); setSelectedCell(null); setIsEraser(false); setWandMode(false); setWandSelection(new Set()); }} className={`px-2.5 py-1.5 text-xs font-bold rounded-lg border transition-all cursor-pointer ${editMode ? 'bg-indigo-500 text-white border-indigo-500' : 'bg-white/[0.05] text-slate-300 border-white/[0.08] hover:bg-white/[0.1]'}`}>{editMode ? '编辑中' : '手动编辑'}</button>
        {editMode && <>
          <span className="w-px h-5 bg-white/10" />
          <button onClick={undo} className="px-2.5 py-1.5 text-xs font-bold rounded-lg border transition-all cursor-pointer bg-white/[0.05] text-slate-300 border-white/[0.08] hover:bg-white/[0.1]" title="撤销 (Ctrl+Z)">撤销</button>
          <button onClick={redo} className="px-2.5 py-1.5 text-xs font-bold rounded-lg border transition-all cursor-pointer bg-white/[0.05] text-slate-300 border-white/[0.08] hover:bg-white/[0.1]" title="重做 (Ctrl+Shift+Z)">重做</button>
          ...
          <div className="hidden sm:flex items-center gap-1.5">
            ...
            <button onClick={() => { setWandMode(!wandMode); setWandSelection(new Set()); }} className={`px-2.5 py-1.5 text-xs font-bold rounded-lg border transition-all cursor-pointer ${wandMode ? 'bg-cyan-500 text-white border-cyan-500' : 'bg-white/[0.05] text-slate-300 border-white/[0.08] hover:bg-cyan-500/20 hover:text-cyan-400 hover:border-cyan-500/30'}`}>魔棒</button>
          ...
          {wandMode && wandSelection.size > 0 && <span className="text-xs text-cyan-400 font-mono">已选{wandSelection.size}格</span>}
          {brushBead && <div className="flex items-center gap-1.5 px-2 py-1 bg-amber-500/10 border border-amber-500/20 rounded-lg"><div className="w-4 h-4 rounded-full border border-white/20" style={{ backgroundColor: brushBead.hex }} /><span className="text-xs font-bold text-amber-400">{brushBead.code}</span><button onClick={() => setBrushBead(null)} className="text-amber-500/70 hover:text-red-400 ml-0.5 text-xs leading-none">✕</button></div>}
        </>}
      </div>
      ...
      <div ref={containerRef} className="flex-1 w-full overflow-hidden flex items-center justify-center relative border border-[#273047] rounded-2xl bg-[#09090B] p-4 group min-h-[320px] md:min-h-[380px]" style={{ cursor: (editMode && !dragMode) ? 'crosshair' : 'grab', touchAction: 'none' }}
