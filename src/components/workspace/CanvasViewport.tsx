...
export default function CanvasViewport({ canvasRef, containerRef, gridWidth, gridHeight, currentPalette, onGeneratePng, onGeneratePdf }: Props) {
  ...
  return (
    <div className={`bg-slate-950 border border-[#1D1D21] rounded-3xl p-6 shadow-2xl flex flex-col relative overflow-hidden min-h-[500px] md:min-h-[500px] transition-all ${mobileTab !== 'canvas' ? 'hidden lg:block' : ''}`}>
      ...
      {editMode && showPalettePanel && (
        <div className="mb-3 p-3 bg-[#0F0F13] border border-white/[0.06] rounded-2xl max-h-[320px] overflow-y-auto">
          {COLOR_GROUPS.map(group => { const groupBeads = currentPalette.filter(b => b.series === group.series); if (groupBeads.length === 0) return null; return (
            <div key={group.series} className="mb-3 last:mb-0"><div className="text-[10px] font-bold text-slate-500 mb-1.5">{group.name}</div><div className="flex flex-wrap gap-1">
              {groupBeads.map(b => (<button key={b.code} onClick={() => { setBrushBead(b); setIsEraser(false); }} className="w-7 h-7 rounded-md border border-white/[0.06] hover:scale-125 hover:z-10 transition-all cursor-pointer relative group/bead" style={{ backgroundColor: b.hex }} title={b.code + ' ' + b.name}><span className="absolute inset-0 flex items-center justify-center text-[7px] font-bold opacity-0 group-hover/bead:opacity-100" style={{ color: (hexToRgb(b.hex).r * 0.299 + hexToRgb(b.hex).g * 0.587 + hexToRgb(b.hex).b * 0.114) > 140 ? '#000' : '#fff' }}>{b.code}</span></button>))}
            </div></div>);})}
        </div>
      )}

      <div ref={containerRef} className="flex-1 w-full overflow-hidden flex items-center justify-center relative border-2 border-dashed border-white/[0.06] rounded-2xl bg-[#09090B] p-4 group min-h-[380px] md:min-h-[380px]" style={{ cursor: editMode ? 'crosshair' : 'grab', touchAction: 'none' }}
