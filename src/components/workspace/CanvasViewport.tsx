...
export default function CanvasViewport({ canvasRef, containerRef, gridWidth, gridHeight, currentPalette, onGeneratePng, onGeneratePdf }: Props) {
  ...
  return (
    <div className={`bg-slate-950 border border-[#1D1D21] rounded-3xl p-6 shadow-2xl flex flex-col relative overflow-hidden min-h-[300px] md:min-h-[500px] transition-all ${mobileTab !== 'canvas' ? 'hidden lg:block' : ''}`}>
      ...
      <div ref={containerRef} className="flex-1 w-full overflow-hidden flex items-center justify-center relative border-2 border-dashed border-white/[0.06] rounded-2xl bg-[#09090B] p-4 group min-h-[240px] md:min-h-[380px]" style={{ cursor: editMode ? 'crosshair' : 'grab', touchAction: 'none' }}
