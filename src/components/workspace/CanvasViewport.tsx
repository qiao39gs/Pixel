...
export default function CanvasViewport({ canvasRef, containerRef, gridWidth, gridHeight, currentPalette, onGeneratePng, onGeneratePdf }: Props) {
  ...
  return (
    <div className={`bg-slate-950 border border-[#1D1D21] rounded-3xl p-6 shadow-2xl flex flex-col relative overflow-hidden min-h-[420px] md:min-h-[500px] transition-all ${mobileTab !== 'canvas' ? 'hidden lg:block' : ''}`}>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {editMode && <>
            ...
            <div className="relative sm:hidden">
              <button onClick={() => setMobileToolsOpen(!mobileToolsOpen)} className="px-2.5 py-1.5 text-xs font-bold rounded-lg border transition-all cursor-pointer bg-white/[0.05] text-slate-300 border-white/[0.08] hover:bg-white/[0.1]">
              ...
              {mobileToolsOpen && (
                <div className="absolute right-0 top-full mt-1.5 bg-[#1A1A1F] border border-white/[0.1] rounded-xl shadow-2xl p-1.5 flex flex-col gap-0.5 min-w-[110px] z-20">
                  <button onClick={() => { setIsEraser(!isEraser); setBrushBead(null); setMobileToolsOpen(false); }} className={`px-3 py-2 text-xs font-bold rounded-lg text-left transition-colors cursor-pointer ${isEraser ? 'bg-red-500/20 text-red-400' : 'text-slate-300 hover:bg-white/[0.06]'}`}>橡皮擦</button>
                  <button onClick={() => { denoise(gridWidth, gridHeight, currentPalette); setMobileToolsOpen(false); }} className="px-3 py-2 text-xs font-bold rounded-lg text-left transition-colors cursor-pointer text-slate-300 hover:bg-white/[0.06]">去杂色</button>
                  <button onClick={() => { setWandMode(!wandMode); setWandSelection(new Set()); setIsEraser(false); setMobileToolsOpen(false); }} className={`px-3 py-2 text-xs font-bold rounded-lg text-left transition-colors cursor-pointer ${wandMode ? 'bg-cyan-500/20 text-cyan-400' : 'text-slate-300 hover:bg-white/[0.06]'}`}>魔棒</button>
                  <button onClick={() => { setShowPalettePanel(!showPalettePanel); setMobileToolsOpen(false); }} className={`px-3 py-2 text-xs font-bold rounded-lg text-left transition-colors cursor-pointer ${showPalettePanel ? 'bg-violet-500/20 text-violet-400' : 'text-slate-300 hover:bg-white/[0.06]'}`}>色板</button>
                </div>
              )}
            </div>
          </>}
          ...
          {editMode && <span className="text-xs text-slate-500 sm:hidden">{brushBead ? '点击/拖拽填充' : isEraser ? '点击擦除' : '长按格子取色'}</span>}
        </div>
      ...
      <div ref={containerRef} className="flex-1 w-full overflow-hidden flex items-center justify-center relative border-2 border-dashed border-white/[0.06] rounded-2xl bg-[#09090B] p-4 group min-h-[320px] md:min-h-[380px]" style={{ cursor: editMode ? 'crosshair' : 'grab', touchAction: 'none' }}
